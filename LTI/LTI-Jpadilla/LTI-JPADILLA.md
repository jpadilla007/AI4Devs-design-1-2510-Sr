...
flowchart LR
  ...
  VectorDB --> MatchingService[Matching and Ranking Service]
  MatchingService --> API
  
  API --> EmailService[Email / Calendar Service]
  
  Monitoring[Monitoring and Logging] --> Grafana[Grafana / Prometheus]
  API --> Monitoring
...
flowchart LR
  subgraph Actors["Actores"]
    ...
    C[Candidate]
    ES[Calendars / Email / HRIS]
  end
  
  LTI[/"LTI ATS\nSistema de gestión de selección"/]
  
  R -->|Usa para publicar vacantes,\nrevisar candidatos y coordinar entrevistas| LTI
  C -->|Aplica a vacantes\ny recibe comunicaciones| LTI
  LTI -->|Sincroniza calendarios,\nenvía emails y exporta datos| ES
  ES -->|Provee disponibilidad y notificaciones| LTI
```
...
flowchart TB
  subgraph MatchingService
    ...
    Reranker["Business Rules and Reranker"]
    Cache["Redis Cache"]
